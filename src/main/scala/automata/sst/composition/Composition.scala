...
case class Composition(printOption: Boolean) {
  ...
  def composeToMonoidSST[Q1, Q2, A, B, C, X, Y](sst1: SST[Q1, A, B, X], sst2: SST[Q2, B, C, Y]) = {
    ...
    def innerDelta(f: Trans)(q2: Q2, xb: Either[X, B]): Option[Q2] = {
      xb match {
        ...
        case Right(b) => sst2.delta.withDefaultValue(None)(q2, b)
      }
    }
    ...
    def innerEta(q2: Q2, xb: Either[X, B]): Option[List[Either[(Q2, X), Update2]]] = {
      xb match {
        ...
        case Right(b) if sst2.eta.contains((q2, b)) => Some(List(Right(sst2.eta(q2, b))))
        case _ => Some(List())
      }
    }
    ...
    def largeDelta(f: Trans, m: Update1): Trans = {                                       //
      sst2.states.flatMap(q => sst1.variables.map(x => (q, x) -> innerDeltaHat(f)(q, m.withDefault(x => List(Left(x)))(x)))).collect {
        case r if r._2.nonEmpty => r._1 -> r._2.get
      }.toMap
    }

    def largeEta(f: Trans, m: Update1): Map[(Q2, X), List[Either[(Q2, X), Update2]]] = {      //
      sst2.states.flatMap(q => sst1.variables.map(x => (q, x) -> innerEtaHat(f)(q, m.withDefault(x => List(Left(x)))(x)))).collect {
        case r if r._2.nonEmpty => r._1 -> r._2.get
      }.toMap
    }

    def eta(q1: Q1, f: Trans, a: A) = largeEta(f, sst1.eta(q1, a))

    def final1(q1: Q1, f: Trans) = sst1.finalMap.get(q1).map(innerEtaHat(f)(sst2.q0, _)).collect {case Some(t) => t}

    def final2(q1: Q1, f: Trans) = sst1.finalMap.get(q1).map(u => innerDeltaHat(f)(sst2.q0, u)).collect {case Some(t) => t}
      .flatMap(sst2.finalMap.get(_))

    if (printOption) println("            Find s0     : " + getTime())
    val initial = (sst1.q0, (for (q2 <- sst2.states; x <- sst1.variables) yield ((q2, x), q2)).toMap)
    ...
    def searchStatesAndDelta: (Set[State], Transition) = {
      def rec(queue: List[State], statesSet: Set[State], transitionMap: Transition): (Set[State], Transition) = {
        queue match {
          case (q :: qs) => {
            val transitions = sst1.delta.filter(r => r._1._1 == q._1).collect{case ((p, a), Some(r)) => (q, a) -> (r, largeDelta(q._2, sst1.eta(q._1, a)))} //map(r => (q, r._1._2) -> (r._2, largeDelta(q._2, sst1.eta(q._1, r._1._2))))
    ...
    val vars = for (q2 <- sst2.states; x <- sst1.variables) yield (q2, x)
    ...
    val dfa0: DFA[Int, A] = DFA(sMap.map(_._2).toSet, deltaMap.map(t => (sMap(t._1._1), t._1._2) -> sMap(t._2)), sMap(initial), f1.map(t => sMap(t._1)).toSet)
    val dfa = dfa0.trim

    if (printOption) println("                Q     : " + dfa.states.size)
    if (printOption) println("                Delta : " + dfa.transition.size)

    if (printOption) println("            Find eta    : " + getTime())
    val etaMap = dfa.transition.map(t => t._1 -> eta(dMap(t._1._1)._1, dMap(t._1._1)._2, t._1._2))

    val sst: SST[Int, A, Update2, (Q2, X)] =
      SST(
        dfa.states,
        vars,
        dfa.transition.collect{case ((p, a), r) => (p,a) -> Option(r)},
        etaMap, dfa.q0,
        dfa.finalStates.map(q => q -> f1(dMap(q))).toMap
      )

    MonoidSST(sst, sst2.variables, f2.map(t => sMap(t._1) -> t._2))
  }
  ...
  /**
    ...
    */
  def guessAlphabet[A](sst: SST[_, A, _, _]): Set[A] = sst.delta.keySet.map(_._2)
  ...
  def convertFromMonoidSST[Q, A, B, X, Y](boundedness: Int, msst: MonoidSST[Q, A, B, X, Y]): SST[Int, A, B, (X, Y, Int)] = {
    ...
    def largeEtaPrime(bone: Bone, m: UpdateM) = {
      val t = for (x <- msst.sst.variables; y <- msst.vars2; k <- 0 to boundedness) yield (x, y, k)
      t.collect {
        case (x, y, k) if m.contains(x) => ((x, y, k), resolveStore(vars2, iotaHom(bone)(m(x)))(y, k))
      }.toMap
    }

    def eta(q: Q, b: Bone, a: A) = largeEtaPrime(b, msst.sst.eta(q, a))

    def final0(q: Q, b: Bone) = {
      msst.sst.finalMap.get(q).flatMap(u => {
    ...
    if (printOption) println("            Find s0     : " + getTime())
    val initial = (msst.sst.q0, (for (x <- msst.sst.variables) yield (x, Update.identityShuffle(msst.vars2))).toMap)
    ...
    def searchStatesAndDelta: (Set[State], Transition) = {
      def rec(queue: List[State], statesSet: Set[State], transitionMap: Transition): (Set[State], Transition) = {
        queue match {
          case (q :: qs) => {
            val transitions = msst.sst.delta.filter(r => r._1._1 == q._1).collect{case ((p, a), Some(r)) => (q, a) -> (r, largeDeltaPrime(q._2, msst.sst.eta(q._1, a)))} //map(r => (q, r._1._2) -> (r._2, largeDeltaPrime(q._2, msst.sst.eta(q._1, r._1._2))))
            val states0 = transitions.toSet
            val states = states0.map(r => r._2).filterNot(statesSet(_))
            rec(qs ::: states.toList, statesSet ++ states, transitionMap ++ transitions)
          }
    ...
    if (printOption) println("            Find vars   : " + getTime())
    val vars = for (x <- msst.sst.variables; y <- msst.vars2; k <- 0 to boundedness) yield (x, y, k)
    ...
    val sMap = states.zipWithIndex.toMap
    val dMap = sMap.map(t => t._2 -> t._1)
    val dfa0: DFA[Int, A] = DFA(sMap.map(_._2).toSet, deltaMap.map(t => (sMap(t._1._1), t._1._2) -> sMap(t._2)), sMap(initial), f.map(t => sMap(t._1)).toSet)
    val dfa = dfa0.trim
    if (printOption) println("                Q     : " + dfa.states.size)
    if (printOption) println("                Delta : " + dfa.transition.size)
    ...
    SST(
      ...
      etaMap,
      dfa.q0,
      dfa.finalStates.map(q => q -> f(dMap(q))).toMap
    )
  }
  ...
  def calcBoundedness[Q, A, B, X](sst: SST[Q, A, B, X]): Int = {
    ...
    if (sst.variables.isEmpty)
      return 0

    val one = (for (x <- sst.variables; y <- sst.variables) yield ((x, y), if (x == y) {
      1
    } else {
      0
    })).toMap
    ...
    def trans(qm: (Q, Map[(X, X), Int]), a: A): (Option[Q], Map[(X, X), Int]) = {
      val (q0, m0) = qm
      (sst.delta(q0, a) , (for ((x, v) <- sst.eta(q0, a); y <- sst.variables) yield ((x, y), v.collect{case Left(z) => m0(z, y)}.sum)))
    }
    ...
    def searchStates: Set[(Q, Map[(X, X), Int])] = {
      def rec(queue: List[(Q, Map[(X, X), Int])], openSet: Set[(Q, Map[(X, X), Int])]): Set[(Q, Map[(X, X), Int])] = {
        queue match {
          case (q :: qs) => {
            val next = sst.delta.filter(r => r._1._1 == q._1).toList.map(r => (trans(q, r._1._2))).collect{case (Some(p), m) => (p, m)}.filterNot(openSet(_))
    ...
    reachables.toSeq.map {case (_, m) => sst.variables.toSeq.map(y => sst.variables.toSeq.map(x => m(x, y)).sum).max}.max
  }
