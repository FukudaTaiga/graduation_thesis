...
case class SSTFactory(charSet: Set[Char]){
  ...
  def replaceFirst(str: String, replacement: String): SST[SST_State, Char, Char, SST_Var] = {
    ...
    val finalMap: Map[SST_State, List[Either[SST_Var, Char]]] = sst0.finalMap + (s_last -> List(Left(v)))

    val delta = sst0.delta + ((SST_State(str.length - 1, sstName), str.last) -> Option(s_last)) ++ charSet.map(ch => (s_last, ch) -> Option(s_last)).toMap

    val eta = sst0.eta ++ charSet.map(ch => (s_last, ch) -> Map(v -> List(Left(v), Right(ch)))).toMap

    SST(states, Set(v), delta, eta, s0, finalMap, charSet)
  }
  ...
  def replaceAll(str: String, replacement: String): SST[SST_State, Char, Char, SST_Var] = {
    ...
    val finalMap = List.range(0, str.length).map(i =>
      states(i) -> (List(Left(v)) ::: str.substring(0, i).toList.map(c => Right(c)))
    ).toMap

    val delta0 = List.range(0, str.length).flatMap(i =>
      charSet.map(c => (states(i), c) -> Option(states(0)))
    ).toMap

    def getBackDelta(i: Int, list: List[Int], res: Map[(SST_State, Char), Option[SST_State]]): Map[(SST_State, Char), Option[SST_State]] = {
      list match {
        case Nil => res
        case j :: rest => getBackDelta(i, rest, res + ((states(i), str.charAt(j)) -> Option(states(j + 1))))
      }
    }

    val delta1 = List.range(1, str.length).flatMap(i => getBackDelta(i, next(i), Map())).toMap

    val delta2 = List.range(0, str.length).map(i =>
      (states(i), str.charAt(i)) -> Option(states((i + 1) % str.length))
    ).toMap
    ...
    //
    def getBackEta(i: Int, list: List[Int], res: Map[(SST_State, Char), Map[SST_Var, List[Either[SST_Var, Char]]]]):
    Map[(SST_State, Char), Map[SST_Var, List[Either[SST_Var, Char]]]] = {
      list match {
        case Nil => res
    ...
    SST(states.toSet, Set(v), delta, eta, s0, finalMap, charSet)
  }
  ...
  def reverse: SST[SST_State, Char, Char, SST_Var] = {
    ...
    val finalMap = Map(s0 -> List(Left(v0)))

    val delta = charSet.map(ch => (s0, ch) -> Option(s0)).toMap

    val eta = charSet.map(ch => (s0, ch) -> Map(v0 -> List(Right(ch), Left(v0)))).toMap

    SST(Set(s0), Set(v0), delta, eta, s0, finalMap, charSet)
  }
  ...
  def insert(idx: Int, string: String): SST[SST_State, Char, Char, SST_Var] = {
    ...
    val delta = List.range(0, idx).flatMap(i =>
      charSet.map(c =>
        (states(i), c) -> (Option(states(i + 1)))
      )
    ).toMap ++ charSet.map(c =>
      (states(idx), c) -> (Option(states(idx)))
    ).toMap
    ...
    SST(states.toSet, vars.toSet, delta, eta, s0, finalMap, charSet)
  }

  def emptySST: SST[SST_State, Char, Char, SST_Var] = {
    val s0 = SST_State(0, "empty")
    SST(Set(), Set(), Map(), Map(), s0, Map(), charSet)
  }
}
